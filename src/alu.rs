...
fn kind_supports(kind: &FuncUnitKind, opcode: Opcode) -> bool {
  match kind {
    FuncUnitKind::IntAlu => matches!(opcode,
      Add | Sub | And | Or | Xor | Sll | Srl | Sra | Slt | Sltu |
      Addi | Andi | Ori | Xori | Slli | Srli | Srai | Slti | Sltiu |
      Lui | Auipc
    ),
    FuncUnitKind::IntMul => false, // no M extension yet
    FuncUnitKind::LoadStore => matches!(opcode,
      Lb | Lh | Lw | Lbu | Lhu |
      Sb | Sh | Sw
    ),
    FuncUnitKind::Branch => matches!(opcode,
...
impl<I: OpaqueInstruction, O: OpaqueResult<I>> Diagnosable for ALU<I, O> {
  fn diagnose(&self) -> DiagNode {
    let pipe_usage: String = self.resource_usage.iter()
      .map(|&u| if u { 'X' } else { '.' })
      .collect();
    let mut children = vec![
      DiagNode::leaf("pipes", format!("{} ({} in-flight)", self.pipes.len(), self.in_progress_instructions.len())),
    ...
    for (i, p) in self.in_progress_instructions.iter().enumerate() {
      children.push(DiagNode::leaf(
        format!("[{i}]"),
        format!("{:?}  age={}/{}  pipe={}", p.instruction.get_opcode(), p.age, p.latency, p._resources_used),
      ));
    }
...
fn compute(opcode: Opcode, i: &ExecInputs) -> u32 {
  ...
  match opcode {
    ...
    Beq  => if i.rs1 == i.rs2                          { i.pc.wrapping_add(imm) } else { i.pc + 4 },
    Bne  => if i.rs1 != i.rs2                          { i.pc.wrapping_add(imm) } else { i.pc + 4 },
    Blt  => if (i.rs1 as i32) <  (i.rs2 as i32)       { i.pc.wrapping_add(imm) } else { i.pc + 4 },
    Bge  => if (i.rs1 as i32) >= (i.rs2 as i32)       { i.pc.wrapping_add(imm) } else { i.pc + 4 },
    Bltu => if i.rs1 <  i.rs2                          { i.pc.wrapping_add(imm) } else { i.pc + 4 },
    Bgeu => if i.rs1 >= i.rs2                          { i.pc.wrapping_add(imm) } else { i.pc + 4 },
  }
}
...
impl<I: OpaqueInstruction, O: OpaqueResult<I>> ALU<I, O> {
  ...
  pub fn new(pipes: Vec<AluPipe>) -> Self {
    let n = pipes.len();
    Self { pipes, in_progress_instructions: vec![], resource_usage: vec![false; n], _phantom: PhantomData }
  }
  ...
  pub fn tick(&mut self) -> Option<O> {
    ...
    for i in 0..self.in_progress_instructions.len() {
      ...
      if instr.latency < instr.age { record = false; }   // Not ready yet
      if instr.age <= max_age { record = false; }       // Not the oldest
