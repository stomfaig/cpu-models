...
impl Diagnosable for SimpleReservationStation {
    fn diagnose(&self) -> DiagNode {
        let mut children = vec![DiagNode::leaf(
            "capacity",
            format!("{}/{}", self.station_entries.len(), self.num_stations),
        )];
...
impl ReservationStation for SimpleReservationStation {
    ...
    fn update(&mut self, rob_tag: usize, val: u32) {
        for entry in &mut self.station_entries {
            if let Operand::Waiting(t) = entry.rs1 {
                if t == rob_tag {
                    entry.rs1 = Operand::Ready(val);
                }
            }
            if let Operand::Waiting(t) = entry.rs2 {
                if t == rob_tag {
                    entry.rs2 = Operand::Ready(val);
                }
            }
