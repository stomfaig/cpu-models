...
use crate::utils::isa::{InstResult, InstrFormat};
...
impl Diagnosable for SimpleReorderBuffer {
    fn diagnose(&self) -> DiagNode {
        ...
        let mut children = vec![
            DiagNode::leaf(
                "size",
                format!("{}/{}", self.buffer.len(), self.buffer.capacity()),
            ),
...
impl ReorderBuffer for SimpleReorderBuffer {
    ...
    fn enqueue(&mut self, instr: &IdRs) -> usize {
        ...
        self.buffer.push(SROBEntry {
            _pc: instr.pc,
            rd: instr.instr.rd,
            format: instr.instr.format,
            ready: false,
            result: None,
        })
    }
    ...
    fn release(&mut self) -> Option<TaggedResult> {
        ...
        let result = rob_entry
            .result
            .expect("ROB entry marked ready without a result");
        Some(TaggedResult {
            rob_tag,
            result: InstResult {
                rd: rob_entry.rd,
                format: rob_entry.format,
                result,
            },
        })
    }
