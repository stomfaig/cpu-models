...
use crate::isa::{DecodedInstr, InstResult, decode, Opcode};
...
impl OpaqueInstruction for InstrFormat {
  ...
  fn get_inputs(&self) -> ExecInputs {
    let rs1 = match self.instr.rs1 { Operand::Ready(v) => v, _ => panic!("rs1 not ready at dispatch") };
    let rs2 = match self.instr.rs2 { Operand::Ready(v) => v, _ => panic!("rs2 not ready at dispatch") };
    ExecInputs { pc: self.instr.pc, rs1, rs2, imm: self.instr.imm }
  }
}
...
impl OpaqueResult<InstrFormat> for ResultFormat {
  fn from_instr_and_result(instr: InstrFormat, result: u32) -> Self {
      ResultFormat { rob_tag: instr.rob_tag, instr: instr.instr, result }
  }
}
...
impl SimpleOOO {
  ...
  pub fn tick(&mut self) {
    ...
    // to put them.
    let stall_frontend = self.reservation_station.stall() || self.reorder_buffer.stall() || self.branch_stall;
    ...
    if let Some(buffer_state) = self.if_id.read() {
      ...
      if let Some(instr) = decoded {
        self.id_rs.stage(IdRs { pc: buffer_state.pc, instr });
      } else {
        if buffer_state.word == 0 { return; }
    ...
    if let Some(instr) = self.reorder_buffer.release() {
      ...
      if Self::writes_rd(instr.result.format) && rd != 0 {
        if let Some(rat_tag) = self.rat[rd] {
          if rat_tag == instr.rob_tag { self.rat[rd] = None; }
        }
  ...
  fn fetch(&mut self) -> IfId {
    ...
    let result = IfId { pc: self.pc, word: u32::from_le_bytes(self.mem[pc..pc+4].try_into().unwrap()) };
    self.pc += 4;
    result
  }

  fn is_branch(opcode: Opcode) -> bool {
    matches!(opcode, Opcode::Beq | Opcode::Bne | Opcode::Blt | Opcode::Bge | Opcode::Bltu | Opcode::Bgeu | Opcode::Jal | Opcode::Jalr)
  }
  ...
  fn writes_rd(format: crate::isa::InstrFormat) -> bool {
  ...
  fn mem_stage(&mut self, instr: ResultFormat) -> ResultFormat {
    ...
    let result = match instr.instr.opcode {
      ...
      Sb | Sh | Sw => {
        let Ready(data) = instr.instr.rs2 else { panic!("store data not ready at mem stage") };
...
impl Diagnosable for SimpleOOO {
  fn diagnose(&self) -> DiagNode {
    let stall = self.reservation_station.stall() || self.reorder_buffer.stall() || self.branch_stall;

    // RAT: only show registers that have a pending mapping
    let rat_entries: Vec<DiagNode> = self.rat.iter().enumerate()
      .filter_map(|(r, slot)| slot.map(|tag| DiagNode::leaf(format!("x{r}"), format!("rob[{tag}]"))))
      .collect();
    ...
    DiagNode::inner("simple_ooo", vec![
      ...
      DiagNode::leaf("mem_rob_release", format!("{:?}", self.mem_rob_release.peek())),
      rat_node,
      self.alu.diagnose(),
