...
fn assemble_line(line: &str, pc: u32, labels: &HashMap<String, u32>) -> Result<u32, AsmError> {
    ...
    match mnemonic {
        ...
        "mv" => {
            let (rd, rs) = reg2(rest)?;
            return Ok(i_type(rd, rs, 0, 0x0, 0b0010011));
        }
        "li" => {
            let (rd, imm) = reg_imm(rest, pc, labels)?;
            return Ok(i_type(rd, 0, imm, 0x0, 0b0010011));
        }
        "j" => {
            let off = resolve(rest, pc, labels)?;
            return Ok(j_type(0, off));
        }
        "jr" => {
            let rs = reg(rest)?;
            return Ok(i_type(0, rs, 0, 0x0, 0b1100111));
        }
        "ret" => return Ok(i_type(0, 1, 0, 0x0, 0b1100111)), // jalr x0, x1, 0
        "not" => {
            let (rd, rs) = reg2(rest)?;
            return Ok(i_type(rd, rs, -1, 0x4, 0b0010011));
        } // xori rd, rs, -1
        "neg" => {
            let (rd, rs) = reg2(rest)?;
            return Ok(r_type(rd, 0, rs, 0x0, 0x20, 0b0110011));
        } // sub rd, x0, rs
        _ => {}
    }
    ...
    match mnemonic {
        // R-type
        "add" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x0, 0x00, 0b0110011))
        }
        "sub" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x0, 0x20, 0b0110011))
        }
        "and" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x7, 0x00, 0b0110011))
        }
        "or" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x6, 0x00, 0b0110011))
        }
        "xor" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x4, 0x00, 0b0110011))
        }
        "sll" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x1, 0x00, 0b0110011))
        }
        "srl" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x5, 0x00, 0b0110011))
        }
        "sra" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x5, 0x20, 0b0110011))
        }
        "slt" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x2, 0x00, 0b0110011))
        }
        "sltu" => {
            let (rd, rs1, rs2) = reg3(rest)?;
            Ok(r_type(rd, rs1, rs2, 0x3, 0x00, 0b0110011))
        }

        // I-type ALU
        "addi" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x0, 0b0010011))
        }
        "andi" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x7, 0b0010011))
        }
        "ori" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x6, 0b0010011))
        }
        "xori" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x4, 0b0010011))
        }
        "slli" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x1, 0b0010011))
        }
        "srli" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x5, 0b0010011))
        }
        "srai" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm | (0x20 << 5), 0x5, 0b0010011))
        }
        "slti" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x2, 0b0010011))
        }
        "sltiu" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x3, 0b0010011))
        }

        // Loads
        "lb" => {
            let (rd, rs1, imm) = reg_mem(rest)?;
            Ok(i_type(rd, rs1, imm, 0x0, 0b0000011))
        }
        "lh" => {
            let (rd, rs1, imm) = reg_mem(rest)?;
            Ok(i_type(rd, rs1, imm, 0x1, 0b0000011))
        }
        "lw" => {
            let (rd, rs1, imm) = reg_mem(rest)?;
            Ok(i_type(rd, rs1, imm, 0x2, 0b0000011))
        }
        "lbu" => {
            let (rd, rs1, imm) = reg_mem(rest)?;
            Ok(i_type(rd, rs1, imm, 0x4, 0b0000011))
        }
        "lhu" => {
            let (rd, rs1, imm) = reg_mem(rest)?;
            Ok(i_type(rd, rs1, imm, 0x5, 0b0000011))
        }

        // Stores
        "sb" => {
            let (rs1, rs2, imm) = reg_mem_store(rest)?;
            Ok(s_type(rs1, rs2, imm, 0x0))
        }
        "sh" => {
            let (rs1, rs2, imm) = reg_mem_store(rest)?;
            Ok(s_type(rs1, rs2, imm, 0x1))
        }
        "sw" => {
            let (rs1, rs2, imm) = reg_mem_store(rest)?;
            Ok(s_type(rs1, rs2, imm, 0x2))
        }

        // Branches
        "beq" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x0))
        }
        "bne" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x1))
        }
        "blt" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x4))
        }
        "bge" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x5))
        }
        "bltu" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x6))
        }
        "bgeu" => {
            let (rs1, rs2, off) = reg_reg_label(rest, pc, labels)?;
            Ok(b_type(rs1, rs2, off, 0x7))
        }

        // Jumps
        "jal" => {
            let (rd, off) = reg_label(rest, pc, labels)?;
            Ok(j_type(rd, off))
        }
        "jalr" => {
            let (rd, rs1, imm) = reg_reg_imm(rest, pc, labels)?;
            Ok(i_type(rd, rs1, imm, 0x0, 0b1100111))
        }

        // Upper immediate
        "lui" => {
            let (rd, imm) = reg_imm(rest, pc, labels)?;
            Ok(u_type(rd, imm, 0b0110111))
        }
        "auipc" => {
            let (rd, imm) = reg_imm(rest, pc, labels)?;
            Ok(u_type(rd, imm, 0b0010111))
        }
...
fn reg(s: &str) -> Result<u8, AsmError> {
    ...
    if let Some(n) = s.strip_prefix('x') {
        n.parse::<u8>()
            .map_err(|_| AsmError::UnknownRegister(s.to_string()))
    } else {
...
fn reg2(s: &str) -> Result<(u8, u8), AsmError> {
    let ops = split_ops(s);
    if ops.len() < 2 {
        return Err(AsmError::BadOperands(s.to_string()));
    }
...
fn reg_mem(s: &str) -> Result<(u8, u8, i32), AsmError> {
    let comma = s
        .find(',')
        .ok_or_else(|| AsmError::BadOperands(s.to_string()))?;
