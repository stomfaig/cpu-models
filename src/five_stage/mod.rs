...
fn default_alu() -> FiveALU {
    FiveALU::new(vec![AluPipe::new(vec![
        FuncUnitKind::IntAlu,
        FuncUnitKind::LoadStore,
        FuncUnitKind::Branch,
    ])])
}
...
impl FiveStageCpu {
    ...
    pub fn tick(&mut self) {
        ...
        if !stall {
            self.if_id.stage(IfId {
                pc: self.pc,
                word: self.mem_read_u32(self.pc),
            });
            self.pc += 4;
        }
        ...
        if let Some(mut latch) = id_ex_snap {
            self.forwarding
                .forward(&mut latch, &ex_mem_snap, &mem_wb_snap);
        ...
        if let Some(latch) = ex_mem_snap {
            ...
            if let Some(w) = write {
                self.apply_mem_write(w);
            }
            self.mem_wb.stage(MemWb {
                instr: latch.instr,
                result,
            });
        }
        ...
        if let Some(latch) = mem_wb_snap {
            ...
            let writes = matches!(
                latch.instr.format,
                InstrFormat::R | InstrFormat::I | InstrFormat::U | InstrFormat::J
            );
            if writes && rd != 0 {
                self.regs[rd as usize] = latch.result;
            }
        }
